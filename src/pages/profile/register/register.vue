              ...
              <div class="userInfo">
                <div class="info">
                  ...
                  <div class="itemInfo">
                    ...
                    <div class="inputFrame">
                      <input type="text" placeholder="请输入验证码" class="input" v-model="user.randomCode"></input>
  ...
  export default {
    ...
    methods: {
      ...
      async checkPhone() {
        ...
        if (this.isSend) {
          if(this.user.phone === ""){
            ...
            await getCode(this.user.phone).then(res=>{
      ...
      async next() {    //下一步
        ...
        //   }
        // }
        if (this.password === this.ensurePassword && this.user.randomCode!=="" && this.user.phone!=="" && this.user.username !== "") {
          ...
          await register(this.user).then(res=>{
            ...
            if (res.data.msg === "SUCCESS") {
              ...
              if (that.response.indexOf("密码") > -1) {
                ...
                that.nameUsed = false         //用户名是否被使用
                that.codeErr = false
              }else if (that.response.indexOf("验证码") > -1){
                that.illegalPassword = false
                that.notQualified = false     //用户名不合法
                that.illegalPhone = false     //非法手机号
                that.nameUsed = false         //用户名是否被使用
